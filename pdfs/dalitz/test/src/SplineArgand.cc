...
int main()
{
	...
	{
		{0.98, std::polar<double>(0.1,0.00)},
		{1.00, std::polar<double>(0.2,0.10)},
		{1.20, std::polar<double>(0.5,0.34)},
		{1.40, std::polar<double>(0.8,0.30)},
		{1.60, std::polar<double>(1.0,0.42)},
	...
	// Argand diagram
	TGraphPolar k_polargraph(k_mass.size(), k_theta.data(), k_radius.data());
	k_polargraph.SetTitle("");
	k_polargraph.SetMarkerStyle(8);
	k_polargraph.SetMarkerSize(1);
	k_polargraph.Draw("AP");
	TGraphPolar polargraph(mass.size(), theta.data(), radius.data());
	polargraph.SetTitle("");
