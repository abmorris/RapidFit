...
namespace Mathematics
{
  ...
  void getBs2PhiPhiAngularFunctions( double & f1
                        ...
                        , const double phi)
        {
                ...
                f7 = 4./9.;
                // f8 ( As_sq )
                //f8 = 0.5*4./3. * ( ct1 + ct2 ) * ( ct1 + ct2 ); // Original
                f8 = 0.5*4./3. * ( ( ct1 * ct1 ) + ( ct2 * ct2 ) );
                // f9 ( Re(AsAss) )
                f9 = 8./3./sqrt(3.) * ( ct1 + ct2 );
                // f10 ( Re(AzAss) )
                f10 = 8./3. * ct1 * ct2;
                // f11 ( Re(ApAss) )
                f11 = 4.*sqrt_2/3. * sin(acos(ct1)) * sin(acos(ct2)) * cos(phi);
                // f12 ( Im(AtAss) )
                f12 = -4.*sqrt_2/3. * sin(acos(ct1)) * sin(acos(ct2)) * sin(phi);
                // f13 ( Re(AzAs) )
                f13 = 8./sqrt(3.) * ct1 * ct2 * ( ct1 + ct2 );
                // f14 ( Re(ApAs) )
                f14 = 4.*sqrt_2/sqrt(3.) * sin(acos(ct1)) * sin(acos(ct2)) * ( ct1 + ct2 ) * cos(phi);
                // f15 ( Im(AtAs) )
                f15 = -4.*sqrt_2/sqrt(3.) * sin(acos(ct1)) * sin(acos(ct2)) * ( ct1 + ct2 ) * sin(phi);
  }
